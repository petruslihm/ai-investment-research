...
개인 계좌, 운영 DB, 실제 LLM 응답 원본을 이식하지 않았습니다. 별도 `etf-radar`의 과거 운영 사례는 [집계·관찰](EVALUATION.md)로 설명합니다. 최신 소스와 합성 재현은 [별도 공개 저장소](https://github.com/petruslihm/etf-radar)에 있으며, 그 테스트 수를 이 저장소의 전체 결과에 합산하지 않습니다. 기존 Git 이력을 재작성하지 않고 개선 내용을 추가합니다.
