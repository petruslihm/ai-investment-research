...
공개 코드에서 재현 가능한 사례와 별도 `etf-radar`에서 관찰한 사례를 구분했습니다. 비공개 사례의 원본 계좌·응답·DB는 포함하지 않습니다.
...
**근거 범위:** 비공개 `screener/portfolio_units.py`, `screener/decision_plan.py` 및 연구 기능 테스트를 2026-09-10에 확인했습니다. 공개본에도 평가 불가 보유의 `null` 보존을 확인하는 별도 데모가 있지만, 두 프로젝트가 동일 코드라는 뜻은 아닙니다.
...
**현재 코드에서 확인한 통제:** 초안 검증 후 `assert_draft_preserved`로 행동, 진입 범위, 추격 한도, 이벤트 조건, 종료 조건, 목표 비중, 현금 관련 필드를 비교합니다. 비교 대상 필드가 달라지면 보존 검사를 통과하지 못합니다.

**근거 범위:** 비공개 `screener/automation.py`와 연구 기능 테스트 6개를 2026-09-10에 확인·실행했습니다. 공개본에서 재현 가능한 검증은 [평가 기록](EVALUATION.md)에 별도로 구분합니다.

**한계:** 지정 필드의 동일성 검사입니다. 원문의 사실성이나 모든 자연어 문장의 의미 일치를 검증하는 기능은 아닙니다. 후속 근거의 단어 중복을 확인하는 기능 역시 사실 재검증과 구분해야 합니다.
